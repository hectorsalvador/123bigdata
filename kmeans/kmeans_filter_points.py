...
class MRInManhattan(MRJob):

    def __init__(self, args):
        MRJob.__init__(self, args)

    def configure_options(self):
        super(MRInManhattan, self).configure_options()
    ...
    def mapper(self, _, line):
        l = line.split(',')
        if l[0] != '"Date/Time"' and l[0] != 'Date/Time':
            time = l[UBER_TIME_COL].strip('"')
            lat1=float(l[UBER_LAT_COL])
            lng1=float(l[UBER_LNG_COL])
        ...
        #         lng2=float(l[TAXI_P_LNG])
            ...
            if in_time_window and point_in_Manhattan(lat1, lng1):
